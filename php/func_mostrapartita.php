...
function mostra_partita($id, $edit, $nuovariga = false) {
	...
	if ($row['PuntiNascosti']) {
		$turni_giocati = $conn->query("SELECT * FROM mani WHERE Partita = $id;")->num_rows;
		$giocatori_attivi = giocatori_attivi_alturno($id, $turni_giocati + 1);
		$giocatori = $giocatori_attivi[0];

		$out .= '<h6><i>Turni disputati:</i> <strong>' . $turni_giocati . '</strong></h6>';
		$out .= '<h6><i>Attualmente in competizione:</i></h6>';
		foreach ($giocatori as $i => $g) {
			$nomi = nomedi($g, true);
			$out .= '<div class="row">';
			$out .= '<div class="col col-sm-4 col-md-3">';
				$out .= ($g == null ? '<i class="bi bi-question-diamond-fill"></i>' : '<a href="giocatori.php?id=' . $g . '">' . nomedi($g) . '</a>');
			$out .= '</div><div class="col">';
				$out .= '<i>dal turno ' . $giocatori_attivi[1][$i] . '</i>';
				if ($edit) {
					$out .= '&nbsp;' . ($giocatori_attivi[1][$i] == 1 ? '<button class="btn btn-sm btn-warning" onclick="primogioc(' . ($i + 1) . ', [\'' . addslashes($nomi[0]) . '\', \'' . addslashes($nomi[1]) . '\']);"><i class="bi bi-pencil-fill"></i></button>' : '<button class="btn btn-sm btn-danger" onclick="modalannullacambio(' . $giocatori_attivi[1][$i] . ', ' . ($i + 1) . ', [\'' . addslashes($nomi[0]) . '\', \'' . addslashes($nomi[1]) . '\']);"><i class="bi bi-x-lg"></i></button>');
				}
			$out .= '</div></div>';
		}
		$out .= '</p>';
		...
		foreach ($partita[0] as $i => $parz) {
			...
			for ($j = 0; $j < 5; $j++) {
				...
				$out .= '<div class="col border-end border-bottom border-primary sfondo' . (!empty($sfondo) ? ' eggcasella" style="background-image: url(\'media/img/gif/' . $sfondo . '.gif\'); cursor: pointer; height: inherit; overflow: hidden;" onclick="tableEgg(this, \'' . $suono . '\');"' : '"') . '>';
					...
					$out .= '<div class="row d-none d-md-flex text-center" style="height: inherit;">';
					...
					$out .= '</div>';
					$out .= '<div class="d-md-none text-center">';
			...
			while ($rowg != null) {
				...
				for ($j = 0; $j < 5; $j++) {
					if ($rowg != null && $rowg['Colonna'] == $j + 1 && $actualrow == $rowg['Inizio']) {
						...
						$out .= '<div class="col pad-alto border-end border-bottom border-primary text-truncate text-center" style="pointer-events: auto; background: var(--sfondo); position: relative; height: ' . 31 . 'px;">';
		...
		if (count($partita[0]) > 0) {
			...
			for ($j = 0; $j < 5; $j++) {
				$out .= '<div class="col bordo2g pad-alto text-center" style="font-size: 20px;"><strong>' . ($totali[$j] > 0 ? '+' : '') . $totali[$j] . '</strong></div>';
			}
			$out .= '</div>';
		}
		$out .= '</div>';
		if (count($partita[0]) > 0) {
			...
			for ($j = 0; $j < 5; $j++) {
				$out .= '<div class="col no-pad text-center"><img src="media/img/Medaglia' . $partita[5][$j] . '.png" height="40px"' . (count($partita[0]) < $minimomedaglie ? ' class="img-bn"' : ' onclick="suonomedaglia(' . $partita[5][$j] . ');"') . '></div>';
			}
			$out .= '</div>';
		}
	}
	...
	if (!$row['PuntiNascosti'] && count($partita[0]) > 1) {
	...
	$out .= checkalias();
	if ($edit && $row['PuntiNascosti'] && count($partita[0]) > 0) {
		$out .= '</div><div class="col-sm-auto text-end"><button class="btn btn-sm btn-outline-primary mb-3" onclick="turno(' . $turni_giocati . ');"><i class="bi bi-pencil-fill"></i> Modifica ultimo turno</button>';
	}
	$out .= '</div><div class="col-lg-2"></div></div>';
	
	if ($edit) {
		$out .= '<div class="row my-3"><div class="col-lg-2"></div>';
		$out .= '<div class="col-sm-6 col-lg-4"><button class="btn btn-lg btn-primary w-100 mb-1" onclick="turno();"><i class="bi bi-patch-plus-fill"></i> Nuovo turno</button></div>';
		$out .= '<div class="col-sm-6 col-lg-4"><button class="btn btn-lg btn-warning w-100 mb-1" onclick="cambio();"><i class="bi bi-person-plus-fill"></i> Cambio giocatore</button></div>';
		$out .= '<div class="col-lg-2"></div></div>';
	}
	
	// Note e foto
	$outf = carousel_foto($id);
	$foto = !empty($outf);
	$outf = '<h3 class="text-primary text-center">Foto ricordo</h3>' . $outf;

	$note = (!empty($row['Note']) ? '<h3 class="text-primary text-center">Note sulle giuocate</h3><p id="note0" style="text-align: justify;">' . $row['Note'] . '</p>' : '<span id="note0"></span>');
	...
	if (!$row['PuntiNascosti'] && count($partita[0]) > 1) {
		...
		$out .= '<h3 class="text-primary text-center mt-4">Classifiche</h3><hr><div class="row">';
		...
		// Chiamate
		$out .= '<div class="col-sm mb-4"><h5><i class="bi bi-chat-dots"></i> Chiamate</h5><p>Vinte: <strong>' . $partita[4][0] . '</strong><br>Perse: <strong>' . $partita[4][1] . '</strong><br>Patte: <strong>' . $partita[4][2] . '</strong><br>In mano: <strong>' . $partita[4][3] . '</strong><br>Con cappotto: <strong>' . $partita[4][4] . '</strong></p></div>';

		// Estremi dei punteggi
		$out .= '<div class="col-sm"><h5><i class="bi bi-chevron-expand"></i> Estremi dei punteggi</h5><p>Massimo: <strong>+' . $partita[4][6] . '</strong><br>Minimo: <strong>' . $partita[4][5] . '</strong></p></div>';

		$out .= '</div>';

		$out .= '<hr class="mt-0"><div class="row">';
...
function mostra_partita_breve($id) {
	...
	$outm = $row['PuntiNascosti'] ? '' : mostra_medaglie($partita);
	$outf = carousel_foto($id);

	if (!empty($outm) && !empty($outf)) {
		$out .= '<div class="row">';
